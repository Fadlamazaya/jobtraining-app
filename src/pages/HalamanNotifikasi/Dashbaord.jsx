...
const PositionBarChart = ({ summary, totalEmployees }) => {
    // Memastikan FO, DO, SL/SPV, dan Manager adalah kategori yang dihitung
    const data = [
...
export default function TrainingSummaryDashboard() {
    ...
    const fetchDataSummary = useCallback(async () => {
        ...
        try {
            // 1. Ambil SEMUA training yang relevan (logika ini tidak perlu diubah)
            ...
            // 2. Ambil Karyawan OPERASIONAL (EKSKLUSI Admin/HR)
            const usersCollection = collection(db, "users");
            
            // Query untuk mendapatkan SEMUA karyawan
            const allUsersSnapshot = await getDocs(usersCollection);
            
            // Filter di sisi klien untuk mengecualikan Admin dan HR
            const usersList = allUsersSnapshot.docs
                .map(doc => ({
                    nik: doc.data().nik,
                    position: doc.data().position,
                    role: doc.data().position || 'N/A' // Asumsi 'position' bisa digunakan sebagai role/identifier
                }))
                // ⭐️ FILTER KARYAWAN NON-OPERASIONAL
                .filter(user => user.role.toUpperCase() !== "ADMIN" && user.role.toUpperCase() !== "HR");
            ...
            usersList.forEach(user => {
                ...
                // Posisi lain (misal: IT, Finance) akan terhitung di totalEmployees tapi tidak di sini.
            });
