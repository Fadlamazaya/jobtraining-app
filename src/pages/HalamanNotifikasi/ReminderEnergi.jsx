...
const isTrainingUpcomingOrCurrent = (start, end) => {
    // Fungsi ini menentukan training mana yang akan diambil/difilter (yang belum selesai di masa lalu)
...
// --- FUNGSI BARU: CEK JIKA TANGGAL SELESAI SUDAH LEWAT (DONE) ---
// Status Done hanya berdasarkan tanggal, mengabaikan jam untuk menghindari masalah zona waktu kecil.
const isTrainingDateDone = (endDateString) => {
    if (!endDateString) return false;
    
    // Konversi tanggal selesai training ke objek Date
    const endDate = new Date(endDateString);
    
    // Dapatkan tanggal hari ini
    const today = new Date();
    
    // Reset jam, menit, detik, milidetik ke 00:00:00 untuk kedua tanggal
    // Ini memastikan perbandingan dilakukan murni berdasarkan tanggal (hari/bulan/tahun).
    today.setHours(0, 0, 0, 0); 
    endDate.setHours(0, 0, 0, 0);

    // Jika tanggal selesai (endDate) lebih kecil dari tanggal hari ini (today), berarti sudah selesai.
    return endDate < today;
};
// -----------------------------------------------------------------

export default function ReminderEnergi() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const correlatedReminders = usersList.map(user => {
                // Filter training yang akan/sedang dijalani user ATAU yang sudah selesai (agar status Done bisa muncul)
                const userTrainings = allRelevantTrainings.filter(training =>
                    training.nik === user.nik
                );
                
                // Ambil training yang paling relevan (kita ambil training terakhir)
                ...
                return {
                    ...
                    trainings: currentTraining ? [currentTraining] : [] // Hanya tampilkan 1 training terbaru
                };
            });
                        ...
                        <tbody>
                            {reminders.length > 0 ? (
                                reminders.map((reminder) => {
                                    ...
                                    // Tombol harus mati jika loading ATAU jika statusnya Done
