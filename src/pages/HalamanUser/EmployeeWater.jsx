...
// Komponen CustomModal disesuaikan dengan tema Cyan/Teal
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100]">
            <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-sm transform transition-all">
                {/* Warna judul modal diubah menjadi Teal */}
                <h3 className={`text-xl font-bold mb-3 ${showConfirmButton ? 'text-red-600' : 'text-teal-600'}`}>{title}</h3>
                ...
                <div className="flex justify-end space-x-3">
                    ...
                    {showConfirmButton && (
                        <button
                            onClick={onConfirm}
                            // Warna tombol konfirmasi diubah menjadi Teal
...
export default function EmployeeWaterTreatment() { // Nama komponen diubah
    ...
    // ⭐️ KONFIGURASI UNTUK WATER TREATMENT ⭐️
    const AREA_KERJA = "Water Treatment Plant"; 
    const UNIT_TITLE = "Unit 2: Water Treatment Plant";
    ...
    const handleCheckboxChange = async (index, tipe, field) => {
        ...
        try {
            ...
            // Mengirim data baru ke Firestore
            await updateDoc(userDocRef, {
                [tipe]: {
                    ...userToUpdate[tipe],
                    [field]: !currentStatus, // Kirim status baru
                },
            });
    ...
    const executeFinalPositionUpdate = async () => {
        ...
        try {
            ...
            setModal({ isOpen: false, type: "", title: "", message: "", user: null });
            fetchData(); // Muat ulang data untuk refleksi perubahan
    ...
    return (
        <div className="p-6 pt-20 bg-gray-50 min-h-screen">
            <div className="bg-white p-6 rounded-xl shadow-2xl max-w-7xl mx-auto">
                ...
                {/* Warna judul diubah menjadi Cyan/Teal */}
                <h2 className="text-3xl font-extrabold mb-2 text-cyan-800 border-b pb-2">Kompetensi</h2>
                ...
                <div className="overflow-x-auto shadow-xl rounded-lg border border-gray-100 mb-12">
                    <table className="min-w-full bg-white table-auto">
                        <thead>
                            {/* Warna header diubah menjadi Cyan/Teal */}
                            <tr className="bg-cyan-800 text-white shadow-lg text-sm uppercase">
                                <th className="border-r border-cyan-700 px-3 py-3 w-12">No</th>
                                <th className="border-r border-cyan-700 px-3 py-3 w-64">Nama / Posisi</th>
                                <th className="border-r border-cyan-700 px-3 py-3">Teknikal</th>
                                <th className="border-r border-cyan-700 px-3 py-3">Safety</th>
                                <th className="border-r border-cyan-700 px-3 py-3">Legal</th>
                                <th className="border-r border-cyan-700 px-3 py-3">Soft Skill</th>
                        ...
                        <tbody>
                            {data.map((row, index) => {
                                ...
                                // Warna latar baris target disesuaikan (Cyan)
                                const row3Class = `border-b-2 border-gray-300 hover:bg-cyan-100 transition duration-200 text-sm ${isTargetSet ? 'bg-cyan-50/80' : 'bg-white'}`;
                                // Warna latar posisi baru disesuaikan
                                const tdPosisiBaruClass = isTargetSet ? 'bg-teal-200/70 text-cyan-800' : 'bg-gray-100';

                                const getKompetensiSekarangClass = (field) => {
                                    // Warna latar kompetensi sekarang disesuaikan (Cyan)
                                    return `border-r border-b px-3 py-2 text-center ${row.kompetensiSekarang[field] ? 'bg-cyan-50/50' : 'bg-red-50/50'}`;
                                };
                                ...
                                const getKompetensiBaruClass = (field) => {
                                    ...
                                    // Warna latar kompetensi baru disesuaikan (Cyan)
                                    return `${baseClass} ${row.kompetensiBaru[field] ? 'bg-cyan-100' : 'bg-red-100/50'}`;
                                };

                                return (
                                    <React.Fragment key={row.id}>
                                        <tr className="border-t border-gray-300 hover:bg-gray-50 transition duration-200">
                                            {/* Warna no. diubah menjadi Cyan */}
                                            <td className="border-r border-b px-3 py-2 text-center bg-gray-50 font-extrabold text-lg text-cyan-800" rowSpan={3}>
                                                {row.no}
                                            </td>
                                            ...
                                            <td className="border-b px-3 py-2 text-center bg-gray-50" rowSpan={3}>
                                                <button
                                                    ...
                                                    // Warna tombol diubah menjadi Teal
                                        ...
                                        <tr className="border-b border-gray-200 hover:bg-cyan-50 transition duration-200 text-sm">
                                            {/* Warna latar kolom posisi sekarang disesuaikan (Cyan) */}
                                            <td className="border-r border-b px-3 py-2 text-xs text-gray-700 bg-cyan-100/50 font-medium">
                                            ...
                                            {Object.keys(row.kompetensiSekarang).map((field) => (
                                                <td key={field} className={getKompetensiSekarangClass(field)}>
                                                    <input
                                                        ...
                                                        // Warna checkbox diubah menjadi Teal
                                                        className="h-4 w-4 rounded text-teal-600 border-gray-400 cursor-pointer"
                                                    />
                                                </td>
                                            ))}
                                        </tr>
                                            ...
                                            {Object.keys(row.kompetensiBaru).map((field) => (
                                                <td key={field} className={getKompetensiBaruClass(field)}>
                                                    <input
                                                        ...
                                                        // Warna checkbox diubah menjadi Teal
                                                        className="h-4 w-4 rounded text-teal-600 border-gray-400 cursor-pointer disabled:cursor-not-allowed"
                                                    />
                                                </td>
                                            ))}
                                        </tr>
